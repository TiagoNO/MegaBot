...
void AdicionaMinerais(CelulaM *CelulaFinal,InfoMineral Info)
{
	...
	NovoMineral->Info.Mineral = Info.Mineral;
...
void AdicionaBase(Celula *CelulaFinal,InfoBase Info)
{
	...
	NovaBase->Minerais->Info.Id = Info.Area->getID();
	NovaBase->Minerais->Info.NumBase = NovaBase->Info.NumBase;
	NovaBase->Minerais->Info.Mineral = Info.Area;
	NovaBase->prox = NULL;
}
...
void ImprimirBases(Celula *Bases)
{
	...
	while(aux != NULL)
	{
		Broodwar->sendText("Base: %i %i %i",aux->Info.CampoMinerais,aux->Info.NumBase,aux->Info.Trabalhadores);
		aux2 = aux->Minerais;
		while(aux2 != NULL)
		{
...
int NaBase(Celula *Bases,Unit *Mineral)
{
	...
	while(aux->prox != NULL)
	{
		if(aux->Info.Area->getDistance(Mineral) <= 700) // pesquisa se o mineral esta proximo de alguma base
		{
			achou = 1;
			break;
		}
	...
	if(aux->Info.Area->getDistance(Mineral) <= 700) // verifica o ultimo na lista
	{
		Broodwar->sendText("Achou");
		achou = 1;
	}
...
TilePosition ProcurarBase(Celula *Bases)
{
	...
	while(aux != NULL)
	{
		...
		{
			aux2 = aux->Minerais->prox->Info.Mineral->getTilePosition();
			return aux2;
		}
		aux = aux->prox;
	}
...
Celula * TaxaTrabalhadores(Celula *Bases,Unit *u)
{
	...
	while(aux != NULL)
	{
		if(aux->Info.Area->getID() == u->getID())
		{
			return aux;
		}
		aux = aux->prox;
  ...
  else
  {
    ...
    for(std::set<Unit*>::const_iterator i=Broodwar->self()->getUnits().begin();i!=Broodwar->self()->getUnits().end();i++)
    {
      ...
      else if ((*i)->getType().isResourceDepot())
      {
		...
		Bases->Info.TrabPorMineral = float(Bases->Info.Trabalhadores/Bases->Info.CampoMinerais);
      }
    }
  }
}
...
void ExampleAIModule::onFrame()
{
  ...
  for(std::set<Unit*>::const_iterator i=Broodwar->self()->getUnits().begin();i!=Broodwar->self()->getUnits().end();i++)
  {
	...
	if ((*i)->getType().isWorker())
    {
		TilePosition Construindo = ProcurarBase(Bases);
		if(Construindo != TilePositions::None)
		{
			int x = Construindo.x() + (rand()%15);
			int y = Construindo.y() + (rand()%15);
			//Broodwar->sendText("Base %i %i",x,y);
			if(Broodwar->self()->getRace() == Races::Zerg)
			{
				if(Broodwar->self()->minerals() > UnitTypes::Zerg_Hatchery.mineralPrice())
				{
					if((*i)->build(TilePosition(x,y),UnitTypes::Zerg_Hatchery))
					{
						Construido = true;
					}
					//				(*i)->move(Position(x,y));
			...
			{
				if(Broodwar->self()->minerals() > UnitTypes::Protoss_Nexus.mineralPrice())
				{
					if((*i)->build(TilePosition(x,y),UnitTypes::Protoss_Nexus))
					{
						Construido = true;
					}
				}
			...
			else if(Broodwar->self()->getRace() == Races::Terran)
			{
				...
				{
					if((*i)->build(TilePosition(x,y),UnitTypes::Terran_Command_Center))
					{
						Construido = true;
					}
				}
		...
		if(!(*i)->isCarryingMinerals() && !(*i)->isCarryingGas())
		{
			...
			{
				int x = Bases->Info.Area->getTilePosition().x();
				int y = Bases->Info.Area->getTilePosition().y();
	...
	if((*i)->getType().isResourceDepot() && Trabalhadores < 60)
	{
		Celula *aux;
  ...
  //Broodwar->sendText("%i %i",Bases->Info.CampoMinerais,Bases->Info.Trabalhadores);
  //ImprimirBases(Bases);
  //ImprimirMineraisBase(Bases,1);
  //ImprimirMineraisBase(Bases,2);
}
